...
public class TicketDAO {
	...
	/*
	 멤버변수선언: Spring-JDBC를 사용하기 위한 멤버변수와 setter를 
	 	정의한다. 또한 트랜잭션 처리를 위한 매니저 클래스의 멤버변수와 
	 	setter를 정의한다.
	 */
	...
	public void buyTicket(final TicketDTO dto) {
		...
		try {
			...
			/*
			 티켓 구매갯수에 대한 DB처리
			 :check 제약조건에 의해 갯수가 5장 초과하는 경우에는
			...
			//티켓갯수가 5장 이하라면 정상처리되어 모든 작업이 실제 
			//테이블에 commit된다.
			...
			//갯수가 초과하면 쿼리에러가 발생하므로 모든 작업은
			//rollback 처리된다.
			System.out.println("제약조건을 위배하여 카드결재와 "
					+ "티켓구매 모두가 취소되었습니다.");
